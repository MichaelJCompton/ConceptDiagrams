 ...
 */
public class FastCurveSet implements Iterable<Integer>, Iterator<Integer>, Serializable {
    ...
    // are they equal if we ignore mask
    public boolean logicalEQ(FastCurveSet other, FastCurveSet mask) {
        FastCurveSet thisMasked = new FastCurveSet(this).mask(mask);

        FastCurveSet otherMasked = new FastCurveSet(other).mask(mask);

        return thisMasked.logicalEQ(otherMasked);
    }
    ...
    public boolean subseteqOF(FastCurveSet other, FastCurveSet mask) {
        ...
        thisMasked.maskOut(mask);

        FastCurveSet otherMasked = new FastCurveSet(other);
        otherMasked.maskOut(mask);
    ...
    public void maskOut(FastCurveSet mask) {
        FastCurveSet maskNOT = new FastCurveSet();

        mask.logicalNOT(maskNOT);
        logicalAND(maskNOT);
    }
